...
- **品牌被收录 2 篇，收录率 28.6%**
- 探针级品牌命中率 13.6%
- **当前收录率与早期阶段分布**:早期阶段 22.6% → 近期 4.8%

::: tip 测量能力是这个系统的核心资产
探针 + 爬虫日志双链路把"AI 是否收录了你的内容"这件事变成可量化的指标。每一篇文章都能回答**它有没有被引用、被谁引用**——这是 GEO 系统区别于"凭感觉写"的关键。
:::

### 爬虫侧观测（2026-08-27）

- Googlebot、bingbot 有**文章级**命中
- Bytespider、Baiduspider、OAI-SearchBot、GPTBot、ClaudeBot、PerplexityBot 近期**还在爬取覆盖期**
...
| AI 收录率 | 28.6%（2026-07-31，累计扫描 33 次） |
...
但 `geo_agent_team.py` 的当前形态是**只读审计**——用 `ast.parse` 校验语法，不执行自动发布。下一阶段把它升级为「带人工审批的半自动编排」。

> 这套编排的设计原则是**确定性工作流**：角色分工、顺序执行、人工审批闸门，不依赖模型自主协商。代码变更 / 内容发布 / 外平台发布三类操作分别需要审批——商业内容发布的风险决定了这一层必须由人兜底。

## 十、路线图

> 系统已经在跑、数据也在积累，计划书的重点是**提升收录质量**，把先验证的能力做厚。

| 阶段 | 目标 | 关键动作 | 状态 |
| --- | --- | --- | --- |
| 近期 | 提升收录质量 | 针对 `self_containment` / `answer_block_quality` 做结构迭代；提升搜狐适配（成功率 69%） | 进行中 |
| 近期 | 半自动升级 | 把 `geo_agent_team` 从「只读审计」升级为「带人工审批的半自动编排」 | 已立项 |
| 中期 | 被国内 AI 抓取 | 主动向国内爬虫提交 / 站点地图优化，让 Bytespider、Baiduspider 抓到正文 | 已规划 |
| 远期 | 内容资产复用 | 模板化跨站复用；质检门禁沉淀为 LLM 评测集 | 设计中 |

::: tip 长效探针的意义
收录数字会波动，关键是有没有**长效测量手段**。计划书里的"主动探针 + 回归基线"作为中期目标，是为了让每个改动都能回答"这次的改动是让收录率升了还是降了"。
:::
...
>
> 跑了 50 天，198 篇内容，121 次真实发布，成功率 80%。每一篇都能用探针和爬虫双链路回答"它有没有被 AI 引用、被谁引用"——这是这套系统持续优化的基础。
